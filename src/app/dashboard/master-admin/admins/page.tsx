...
import { Users, PlusCircle, Edit3, Trash2, Loader2, Building, XCircle } from "lucide-react";
...
export default function MasterAdminManageAdminsPage() {
  ...
  const fetchAdminsAndSchools = useCallback(async () => {
    ...
    const [adminsResult, schoolsResult] = await Promise.all([ getSchoolAdmins(), getSchools() ]);
    ...
    else toast({ variant: "destructive", title: "Failed to load schools", description: schoolsResult.error || "Could not fetch school list." });
    setIsLoadingSchools(false);
  }, [toast]);

  useEffect(() => { fetchAdminsAndSchools(); }, [fetchAdminsAndSchools]);
  ...
  return (
    <div className="space-y-6">
                ...
                <FormField control={form.control} name="schoolId" render={({ field }) => (
                  <FormItem><FormLabel className="flex items-center"><Building className="mr-2 h-4 w-4 text-muted-foreground" />Assign to School</FormLabel>
                    <Select onValueChange={field.onChange} value={field.value} disabled={isSubmitting || isLoadingSchools}>
                      <FormControl><SelectTrigger><SelectValue placeholder={isLoadingSchools ? "Loading schools..." : "Select a school"} /></SelectTrigger></FormControl>
                      <SelectContent>
      ...
      <Card>
        <CardHeader><CardTitle>Existing School Administrators</CardTitle></CardHeader>
        <CardContent>
          {isLoadingAdmins ? (<div className="flex items-center justify-center py-4"><Loader2 className="h-8 w-8 animate-spin text-primary" /><p className="ml-2">Loading administrators...</p></div>)
          : admins.length > 0 ? (
          <Table>
