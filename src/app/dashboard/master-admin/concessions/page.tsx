...
import { TicketPercent, PlusCircle, Trash2, Loader2, Building, User, CalendarFold, Search, Info, DollarSign } from "lucide-react";
  ...
  useEffect(() => {
    ...
    if (storedUser && storedUser !== "undefined" && storedUser !== "null") {
      try {
        ...
        if (parsedUser && (parsedUser.role === 'masteradmin' || parsedUser.role === 'superadmin') && parsedUser._id) {
    ...
    async function fetchInitialSchools() {
      ...
      if (schoolsResult.success && schoolsResult.schools) setSchools(schoolsResult.schools);
      else toast({ variant: "destructive", title: "Error", description: schoolsResult.message || "Failed to load schools." });
    ...
    <div className="space-y-6">
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <FormField control={form.control} name="schoolId" render={({ field }) => (
                  <FormItem><FormLabel className="flex items-center"><Building className="mr-2 h-4 w-4 text-muted-foreground"/>Select School</FormLabel>
                    <Select onValueChange={(value) => { field.onChange(value); setSelectedSchoolId(value); }} value={field.value} disabled={isSubmitting || isLoadingSchools || schools.length === 0}>
                      <FormControl><SelectTrigger><SelectValue placeholder={isLoadingSchools ? "Loading schools..." : (schools.length === 0 ? "No schools available" : "Select a school")} /></SelectTrigger></FormControl>
                      <SelectContent>{schools.map((school) => (<SelectItem key={school._id} value={school._id.toString()}>{school.schoolName}</SelectItem>))}</SelectContent>
                    </Select><FormMessage />
                  </FormItem>
                )}/>
                <FormField control={form.control} name="studentId" render={({ field }) => (
                  <FormItem><FormLabel className="flex items-center"><User className="mr-2 h-4 w-4 text-muted-foreground"/>Select Student</FormLabel>
                    <Select onValueChange={field.onChange} value={field.value} disabled={isSubmitting || isLoadingStudents || studentsInSchool.length === 0 || !selectedSchoolId}>
                      <FormControl><SelectTrigger><SelectValue placeholder={!selectedSchoolId ? "Select school first" : isLoadingStudents ? "Loading students..." : (studentsInSchool.length === 0 ? "No students in school" : "Select a student")} /></SelectTrigger></FormControl>
                      <SelectContent>{studentsInSchool.map((student) => (<SelectItem key={student._id!.toString()} value={student._id!.toString()}>{student.name} ({student.classId || 'N/A'})</SelectItem>))}</SelectContent>
                    </Select><FormMessage />
                  </FormItem>
                )}/>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <Input id="academicYearFilter" placeholder="e.g., 2023-2024" className="w-full sm:w-[180px]" value={academicYearFilter} onChange={(e) => setAcademicYearFilter(e.target.value)} disabled={isLoadingConcessions || !selectedSchoolId}/>
              <Button variant="outline" size="icon" onClick={() => fetchConcessionsForSchool(selectedSchoolId, academicYearFilter)} disabled={isLoadingConcessions || !selectedSchoolId}><Search className="h-4 w-4" /></Button>
            </div>
          </div>
        ...
        <CardContent>
          ...
          : !selectedSchoolId ? (<p className="text-center text-muted-foreground py-4">Please select a school to view concessions.</p>)
